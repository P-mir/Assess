...
class Model():
    ...
    2 classes inherit: Regressor and Classfier classes
    '''
    @staticmethod
    def getModel(task,n_jobs=1):
        if task == 'classification':
            return Classifier(n_jobs)
        elif task == 'regression':
            return Regressor(n_jobs)
...
#             grid = [{'max_depth': [5,10, None], 'n_estimators': [n_trees]}]
        ...
        grid = [{'max_depth': [5,8], 'n_estimators': [200]}]
        ...
        self.fitted_model = self.fitted_cv_model.best_estimator_ # best model found during grid search
    ...
    def compute_metrics_and_graphs(self, pred, actual,output_path = "outputs/plots/mlflow_artifacts"):
        ...
        '''
        metrics = dict()
#         metrics['accuracy_train'] = accuracy_score(actual, pred)
...
class Regressor(Model):
    ...
    def fit(self,x,y):
        ...
        self.model = GridSearchCV(RandomForestRegressor(criterion='mse', n_jobs=self.n_jobs), grid, scoring='neg_mean_squared_error')
        self.fitted_cv_model = self.model.fit(x,y)
        
        self.hyperparameters = self.fitted_cv_model.best_params_
        self.validation_rmse = np.sqrt(-self.fitted_cv_model.best_score_)
        self.fitted_model = self.fitted_cv_model.best_estimator_ # best model found during grid search
        return self.fitted_model
    ...
    def compute_metrics_and_graphs(self, pred, actual,output_path = "outputs/plots/mlflow_artifacts"):
        ...
        metrics = dict()
        metrics['r2_test'] = r2_score(actual, pred)
        metrics['rmse_test'] = mean_squared_error(actual, pred, squared=False)
        metrics['rmse_validation'] = self.validation_rmse
#         metrics['auc (test)'] = roc_auc_score(actual, pred)
...
def explain(x, model,task, path = "outputs/plots/mlflow_artifacts/shap",n_features = 5):
    ...
    feature_importance = pd.DataFrame(list(zip(x.columns, sum(vals))), columns=['feature','feature_importance_vals'])
    ...
    #     plt.savefig("plots/shap/interaction_plots/interaction_plot.png",dpi=150, bbox_inches='tight')
        ...
        for feature in features_names:
            ...
            shap.dependence_plot(feature, shap_values, x,
                                 ...
                                 show=False)
            plt.savefig("{}/dependance_plots/{}_dependance_plot.png".format(path,feature),dpi=150, bbox_inches='tight')
            plt.clf()
